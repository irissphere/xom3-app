...
export async function GET() {
  ...
  const snapshot = loadHeartbeat("scraper", tenant);
...
export async function POST(req: Request) {
  ...
  const key = (body?.key || body?.scraperKey || "").toString().trim();
  ...
  const snapshot = upsertHeartbeatEntry("scraper", tenant, key, update);
  return NextResponse.json({ ok: true, tenant, snapshot }, { headers: { "Cache-Control": "no-store" } });
}
