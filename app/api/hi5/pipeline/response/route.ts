...
export async function POST(req: Request) {
  ...
  const record = updateGoldenRecord(tenant, id, {
    step: "response_positioned",
    responseStatus: body?.responseStatus ?? "positioned",
    lastError: null,
  });

  if (!record) return NextResponse.json({ ok: false, error: "not_found" }, { status: 404 });

  const respWebhook = process.env.HI5_N8N_RESPONSE_WEBHOOK_URL;
  const webhook = respWebhook
    ? await triggerWebhook(respWebhook, { tenant, pipelineId: id, ...body })
    : { ok: false, status: 0 };
  ...
  return NextResponse.json(
    {
      ...
      record,
      webhook: {
        configured: Boolean(respWebhook),
        triggered: webhook.ok,
        status: respWebhook ? webhook.status : null,
      },
    },
